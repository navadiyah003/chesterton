...
class GlobalNetworkIntegratedController extends Controller
{
    public function index(Request $request)
    {
        if($request->global_int_service_search){
                $integrated_service = GlobalNetworkIntegrated::where('global_network_country','like','%'.$request->global_int_service_search.'%')
                                                            ->orWhere('global_network_integrate_title','like','%'.$request->global_int_service_search.'%')
                                                            ->orWhere('global_network_integrate_description','like','%'.$request->global_int_service_search.'%')
                                                            ->paginate(6);
                ...
                {   
                    $output .= '<tr '.'id='.$item->id.'>
                                ...
                                <td class="text-truncate" style="max-width: 150px;"> '.$item->global_network_country.'</td>
                                <td class="text-truncate" style="max-width: 150px;"> '.$item->global_network_integrate_title.'</td>
                                ...
                                <td > '.'<a class="btn btn-primary btn-md" href="edit-global-integrated-service/'.$item->id.'">'.'<i class="fas fa-edit"></i>'.' Edit </a>
                                                    '.'<button type="button" class="btn btn-danger btn-md"
                                                            onclick="delete_global_integrated_service(this)" data-id="'.$item->id.'" id="global_integrated_service_delete"><i class="fas fa-trash-alt"></i>'.' Delete </button>
                                                                   '.'</td>
                                </tr>';  
                                
                    $data[] = $item['global_network_country'];                   
                }
        ...
        return view('admin.global-network-integrated-service.global-integrated-service-list',compact('global_integrated_service'));
    }
    ...
    public function edit($id)
    {
        ...
        $globalNetwork = GlobalNetwork::get();
        return view('admin.global-network-integrated-service.edit-global-integrated-service',compact('integrated_service','globalNetwork'));
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if($int_service_id != '')
        {
           ...
           return redirect('admin/global-integrated-service-list')->with('success','Integrated service has been deleted successfully.'); 
        }
    }
